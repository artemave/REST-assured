module RestAssured
  class Response
    ...
    def self.find_matching_double(method, fullpath)
      ...
      active_doubles_for_verb.select {|d| !d.fullpath.blank? && d.fullpath == fullpath}.first ||
          active_doubles_for_verb.select {|d| !d.pathpattern.blank? && Regexp.new(d.pathpattern).match(fullpath)}.last
    end
